...
export default function Home() {
  ...
  const getTokensToBeClaimed = async () => {
    try {
      ...
      if (balance === zero) {
        ...
        // if balance != 0 => the address has some NFT tokens
        ...
        for (var i = 0; i < balance; i++) {
          ...
          // tokenIdsClaimed is a mapping being accessed here like a f(arg)
      ...
      console.error(err);
      setTokensToBeClaimed(zero);   // bcz error just occured
    }
  };
  ...
  const getBalanceOfCryptoDevTokens = async () => {
    try {
      ...
      console.error(err);
      setBalanceOfCryptoDevTokens(zero);    // set to 0 bcz error occured
    }
  };
  ...
  const mintCryptoDevToken = async (amount) => {
    try {
      ...
      await getBalanceOfCryptoDevTokens();  // returns the balance of CDTokens held by the signer
      await getTotalTokensMinted();   
      await getTokensToBeClaimed();   // returns the count of NFT tokens to be claimed
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const withdrawCoins = async () => {
    try {
      ...
      const tx = await tokenContract.withdraw();  // withdraw() here only did 2 checks
      setLoading(true);
      await tx.wait();
      setLoading(false);
      await getOwner();   
      // So, why are we calling getOwner here now?? onlyOwner already checked in withdraw()
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    if (!walletConnected) {
      ...
      getBalanceOfCryptoDevTokens();    // balanceOf(owner), not "left" CD to be minted
